...
function FieldValue({ value }) {
      ...
      <ul className="list-disc list-inside space-y-1">
        {value.map((item, i) => (
          <li key={i} className="text-gray-700 text-sm">
            {typeof item === 'object'
              ? item.etapa || item.descricao || item.termo || JSON.stringify(item)
              : String(item)}
          </li>
        ))}
      </ul>
  ...
  // Object with a lista property (SessaoComLista)
  if (typeof value === 'object' && value.lista !== undefined) {
    const items = value.lista || [];
    return (
      <div>
        {value.descricao && (
          <p className="text-gray-600 text-sm mb-2 italic">{value.descricao}</p>
        )}
        {items.length === 0 ? (
